...
// .wrangler/tmp/bundle-CSqIl8/checked-fetch.js
...
// .wrangler/tmp/bundle-CSqIl8/strip-cf-connecting-ip-header.js
...
function isOriginAllowed(origin, allowed) {
  ...
  if (allowed.includes("*"))
    return true;
  return allowed.some((allowed2) => {
    if (allowed2.startsWith("*.")) {
      const domain = allowed2.slice(2);
      return origin.endsWith(domain);
    }
    return origin === allowed2;
  });
}
__name(isOriginAllowed, "isOriginAllowed");
function createCorsHeaders(origin, config = DEFAULT_CONFIG) {
  const headers = {
...
var PROVIDER_PRIORITY = [
  "openrouter",
  // Full model access via OpenRouter
  "cerebras",
  // Very fast, free tier
  "mistral",
  // Free tier available
...
__name(getAvailableProviders, "getAvailableProviders");
...
function getModelForProvider(requestedModel, provider, isVirtualModel = false) {
...
__name(getModelForProvider, "getModelForProvider");

// src/lib/response.ts
function jsonError(message, type = "server_error", status = 500) {
  return new Response(
    JSON.stringify({
      error: {
        message,
...
function getProviderUrl(provider, env) {
...
__name(getProviderUrl, "getProviderUrl");
async function tryProvider(provider, body, env, isVirtualModel) {
...
// .wrangler/tmp/bundle-CSqIl8/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-CSqIl8/middleware-loader.entry.ts

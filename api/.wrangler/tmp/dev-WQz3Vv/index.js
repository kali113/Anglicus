...
// .wrangler/tmp/bundle-psdxx8/checked-fetch.js
...
// .wrangler/tmp/bundle-psdxx8/strip-cf-connecting-ip-header.js
...
function transformRequestBody(provider, body) {
  if (provider === "cohere") {
    return {
...
__name(transformRequestBody, "transformRequestBody");
async function handleChatCompletions(request, env) {
  try {
    ...
    const providerPriority = [
      ...
      // Free tier
      "groq",
      // Fast, free tier (but may have issues)
      "nvidia",
      // Free tier
      "cohere",
      // Has free tier
    ...
    if (availableProviders.length === 0) {
      return new Response(
        JSON.stringify({
          error: {
            message: "No API keys configured. Please configure at least one provider.",
            type: "server_error"
          }
        }),
    ...
    for (const provider of providersToTry) {
      ...
      try {
        ...
        const modelForProvider = provider === requestedProvider ? body.model : DEFAULT_MODEL_BY_PROVIDER[provider];
...
async function handleListModels(_request, _env) {
      ...
      html: `
        ...
        body.message
				...
				<p style="color: #666; font-size: 14px;">
					...
					<strong>Time:</strong> ${(/* @__PURE__ */ new Date()).toISOString()}
				</p>
			`
    };
...
// .wrangler/tmp/bundle-psdxx8/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-psdxx8/middleware-loader.entry.ts

<script lang="ts">
	...
	import type { UserProfile, EnglishLevel, LearningGoal } from '$lib/types/user.js';
...
Make sure the correctAnswer matches exactly one of the options.`;
			...
			const response = await getCompletion([
				{ role: 'system', content: 'You are an expert English teacher. Generate valid JSON only. Always respond in English.' },
...
<div class="placement-test">
	{#if step === 1}
		<div class="step">
			...
			<p>Vamos a evaluar tu nivel de inglés con un test rápido y personalizado.</p>
	...
	{:else if step === 2}
		<div class="step">
			...
			/>
			<div class="actions">
				<button class="btn primary" onclick={() => step = 3} disabled={!userName.trim()}>
					Continuar
				</button>
			</div>
		</div>
	...
	{:else if step === 3}
		<div class="step">
			...
			<div class="actions">
				<button class="btn primary" onclick={startTest} disabled={userGoals.length === 0}>
					Iniciar Test
				</button>
			</div>
		</div>
			...
			{:else if showResults}
				<div class="step results-step">
					...
					<div class="score-summary">
						<p>Respuestas correctas: {answers.filter((a, i) => a === questions[i]?.correctAnswer).length} / {questions.length}</p>
					</div>
			...
			{:else}
				...
				{#if questions[currentQuestion]}
					<div class="question-card">
						<div class="difficulty-badge">{questions[currentQuestion].difficulty}</div>
